...
<?php
	...
	while($row=$sth2->fetch(PDO::FETCH_ASSOC)){
		...
		';
		echo '<p>'.$song_name.'</p>';
		echo '<select name="list_name" onclick="bubble()" form="'.$song_name.'">';
		foreach($arr as $value){
			echo '<option value='."'$value'".'>'.$value.'</option>';
		}
		echo '</select>';
		echo '<form method="POST" onclick="bubble()" id="'.$song_name.'"action="add_to_list.php?song_name='.$song_name.'"><input type="submit" value="add to list"></form>';
		echo '</div>';
	}
?>
<script>
